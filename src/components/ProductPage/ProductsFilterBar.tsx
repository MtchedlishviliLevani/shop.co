...
import { ProductsFilterBarProps } from "../../types";

function ProductsFilterBar({ categoryList, typeList, onClose, onApplyFilter }: ProductsFilterBarProps) {
    ...
    function handleTypeCheckBox(e: React.ChangeEvent<HTMLInputElement>, type: string) {
    ...
    return (
        <div className="xl:static">
            <div className="xl:border-solid xl:border-primary/10 xl:border-[1px] bg-white rounded-[20px] w-full py-[20px] px-[24px] flex flex-col gap-[24px]">
                <div className="flex items-center justify-between">
                    ...
                    <div className="flex gap-2 items-center">
                        ...
                        <svg className="hidden xl:inline" width="22" height="20" viewBox="0 0 22 20" fill="rgba(0, 0, 0, 0.4)" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <button
                            onClick={onClose}
                            className="cursor-pointer xl:hidden w-[20px] font-bold"
                        >
                            ✕
                        </button>
                    </div>
                ...
                <div className="flex-col flex gap-[20px]">
                    <div className="flex justify-between items-center">
                        <h2 className="xl:text-[20px] font-bold">Product Types</h2>
                    </div>

                    <div className="flex flex-col gap-4">
                        {typeList.map((type, index) => (
                            <div key={index} className="flex justify-between items-center">
                                <label className="capitalize xl:text-[16px] text-primary/60">{type}</label>
                                <input
                                    checked={selectedTypes.includes(type)}
                                    onChange={(e) => handleTypeCheckBox(e, type)}
                                    type="checkbox"
                                />
                            </div>
                        ))}
                ...
                {/* Colors */}
                <ColorFilter onColorChange={setSelectedColors} />

                {/* Sizes */}
                <SizeFilter onSizeChange={setSelectedSizes} />
